...
char xd = 'r', yd = 'u';
void printString(char print[], int Stringlenght, double stringX, double stringY,
		int size) {
...
void Display() {
	if (gameOver == 0) {
		...
		for (i = 0; i < AMMO; i++) {
			if (fireVisible[i] == 1
					&& (x < xfire[i] + 0.09 && x > xfire[i] - 0.09)
					&& y + 0.7  > yfire[i] && y + 0.5 < yfire[i] ) {
				if (powercom > -0.4) {
		...
		for (i = 0; i < AMMO2; i++) {
			if (comfirevisible[i] == 1
					...
					&& ybat - 0.3 > ycomfire[i] && ybat - 0.5 < ycomfire[i] ) {
				if (powercom2 > -0.4) {
	...
	if (gameOver == 2) {
		...
		glClear(GL_COLOR_BUFFER_BIT);
		printString("SPACE BATTLE", 12, -0.7, 0.8, 150);
		printString("Developer : Ankush Deshmukh",
				strlen("Developer : Ankush Deshmukh"), -1.0, 0.6, 60);
		printString("Producer : Ankush Deshmukh",
				strlen("Producer : Ankush Deshmukh"), -1.0, 0.4, 60);
